...
int V4L2Camera::enqueueRequest(
  ...
  std::future<int> handle = std::async(std::launch::async,
      &v4l2_camera_hal::V4L2Wrapper::EnqueueRequest, device_.get(), request);
  res = handle.get();
  if (res) {
...
bool V4L2Camera::dequeueRequestBuffers() {
  ...
  int res = device_->DequeueRequest(&request);
  if (!res) {
    if (request) {
      completeRequest(request, res);
    }
...
int V4L2Camera::setupStreams(camera3_stream_configuration_t* stream_config) {
  ...
  if (stream_config->num_streams > 1) {
    ...
    for (uint32_t i = 1; i < stream_config->num_streams; ++i) {
      ...
      width = std::max(width, stream->width);
      height = std::max(height, stream->height);
      bool ok = stream->format == format;
      // We can decode the HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED to the HAL_PIXEL_FORMAT_BLOB one.
      if (!ok) {
        ok = ok || (stream->format == HAL_PIXEL_FORMAT_BLOB && format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED);
        ok = ok || (format == HAL_PIXEL_FORMAT_BLOB && stream->format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED);
        // Formats are different and we can decode, but the driver can't.
        if (ok && format == HAL_PIXEL_FORMAT_BLOB) {
          // So we set appropriate format which will be later passed to the driver.
          format = HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED;
        }
      }
      if (!ok) {
        // If different dimmentions or we can't convert formats.
        HAL_LOGE(
