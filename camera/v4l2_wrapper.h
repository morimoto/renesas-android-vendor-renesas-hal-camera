...
class V4L2Wrapper {
 ...
 private:
  ...
  // Lock protecting use of the buffer tracker and queue of capture requests.
  std::mutex buffer_lock_;
  // Lock protecting use of the queue of capture requests.
  ...
  std::vector<std::shared_ptr<arc::V4L2FrameBuffer> > buffers_;
  // Queue of the framework's requests. Must be sorted by frame number.
  std::queue<std::shared_ptr<default_camera_hal::CaptureRequest> > requests_;
  const size_t desired_buffers_;

  std::atomic<bool> is_turned_;
